...
import { Box, Text, type Component, type Focusable } from "@earendil-works/pi-tui";
 ...
 */
async function promptSecretTUI(
    ...
    (tui, theme, _kb, done) => {
      ...
      const comp: Component & Focusable = {
        ...
        render(width: number) {
          ...
          if (note) lines.push(" ".repeat(pad) + theme.fg("dim", `      ${note}`));
          lines.push("");
          const dots = Math.min(value.length, boxW - 8);
          const mask = "•".repeat(dots);
          const cursor = value.length <= boxW - 8 ? "█" : "";
          const inner =
            "  " + mask + cursor + " ".repeat(Math.max(1, boxW - 6 - dots - (cursor ? 1 : 0)));
 ...
 */
async function collectViaModal(ctx: ExtensionContext, params: RequestArgs, signal?: AbortSignal) {
...
function registerPiTool(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    renderCall(args, theme) {
      ...
      if (args.tunnel) text += theme.fg("dim", " · tunnel");
    ...
    renderResult(result, options, theme) {
      ...
      if (names) box.addChild(new Text(theme.fg(statusColor, `     ${names}`)));
      if (options.expanded) {
        box.addChild(new Text(""));
        if (details.url) box.addChild(new Text(theme.fg("dim", `     Link: ${details.url}`)));
        if (details.out) box.addChild(new Text(theme.fg("dim", `     Saved to: ${details.out}`)));
        if (details.message) box.addChild(new Text(theme.fg("muted", `     ${details.message}`)));
        box.addChild(new Text(""));
